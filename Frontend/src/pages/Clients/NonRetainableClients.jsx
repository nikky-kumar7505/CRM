...
import { FiUserX, FiPhone } from "react-icons/fi";

const NON_RETAINABLE_SERVICES = ["web_development", "video_editing"];

const NonRetainableClients = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      const filtered = res.data.data.filter((c) =>
        NON_RETAINABLE_SERVICES.includes(c.service_type)
      );
      setClients(filtered);
    } catch (error) {
    ...
    <div className="page-container">
      <div className="page-header">
        <h1 className="page-title" style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <FiUserX style={{ color: "#dc3545" }} /> Non-Retainable Clients
        </h1>
      </div>

      <div className="card" style={{ marginBottom: "16px", padding: "14px 20px" }}>
        <p style={{ fontSize: "13px", color: "#797e88", margin: 0 }}>
          ⚠️ <strong>Non-retainable</strong> = Web Development & Video Editing (one-time services).
        </p>
      </div>
      ...
      <div className="table-container">
        <table className="data-table">
          ...
          <tbody>
            {clients.length === 0 ? (
              <tr><td colSpan="6" style={{ textAlign: "center", padding: "40px" }}>No non-retainable clients</td></tr>
            ) : (
              clients.map((c) => (
                <tr key={c._id}>
...
export default NonRetainableClients;